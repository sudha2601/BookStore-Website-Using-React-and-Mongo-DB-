import React from 'react'
import { useState, useRef ,useEffect} from 'react'
import { useForm } from "react-hook-form"
import axios from "axios"
import { Link } from 'react-router-dom'
import { useNavigate } from 'react-router-dom'


const Login = () => {
  let navigate=useNavigate()
  useEffect(()=>{
    let auth=localStorage.getItem("User")
    if(auth){
        navigate("/")
    }
},[])
  const {
    register,
    handleSubmit,
    reset,
    watch,
    formState: { errors, isSubmitting },
  } = useForm()


  let [seal, newseal] = useState(false)
  let [existing, newexisting] = useState(false)

  const onSubmit = async (data) => {


    try {
      const c = await axios.post("https://bookstore-website-backend.onrender.com/register", data)
      reset({
        Email: "",
        password: "",
        Contact_no:"",
        Name:""

      })
      console.log(c.data)
      if (c.data == "new user") {
        alert("Registered Successfully,Go to Login page")
        navigate("/login")
        
        

      }
      else {
        alert("Already have an account,Login")
        navigate("/login")
        
      }
    }
    catch (error) {
      console.log(error)

    }


  }





  return (
    <>
      <div className='container flex  justify-center h-screen w-full' style={{
        backgroundImage: 'url("data:image/jpeg;base64,")',
        backgroundRepeat: 'no-repeat',
        backgroundSize: 'cover'
      }}>
        <center className='bg-gray-200 rounded-lg border-2 border-black h-fit p-4 m-4 w-1/4'>
          <div className='text-center text-xl font-semibold'>Register</div>
          {isSubmitting && <p>Loading</p>}

          <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col m-2 pt-5 gap-2'>
            <input  {...register("Name", { required: true, message: "Name is required" })} placeholder='Name' className='rounded-lg border-2 border-black h-11 m-2 ' />
            {errors.Name && <p className='text-red-500'>Name is required</p> }
            <input  {...register("Contact_no", { required: true, message: "Contact no. is required" })} placeholder='Contact no.' className='rounded-lg border-2 border-black h-11 m-2 ' />
            {errors.Contact_no && <p className='text-red-500'>Contact no is required</p> }
            <input  {...register("Email", { required: true, message: "Gmail ID is required" })} placeholder='Email ID' className='rounded-lg border-2 border-black h-11 m-2 ' />
            {errors.Email && <p className='text-red-500'>Gmail ID is required</p> }
            <input  {...register("password", { required: { value: true, message: "password is required" }, minLength: { value: 7, message: "minimum length 7 is required" } })} type="password" placeholder='password' className='rounded-lg border-2 border-black h-11 m-2' />
            {errors.password && <p className='text-red-500'>{errors.password.message}</p>}
            <input disabled={isSubmitting} type='submit' className='bg-green-400 rounded p-2 cursor-pointer ' value="Register" />
          </form>
          <p>Already have an account? </p><div><Link className='underline cursor-pointer' to={"/login"}>Login here</Link></div>
          {seal && <p>Congratulations,You have been registered successfully</p>}
          {existing && <p>You have already registered</p>}

        </center>
      </div>
    </>
  )
}

export default Login